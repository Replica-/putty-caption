...
#define f(i,c,w,u,width,h) \
    fonts[i] = CreateFont (h, width, 0, 0, w, FALSE, u, FALSE, \
			   c, OUT_DEFAULT_PRECIS, \
		           CLIP_DEFAULT_PRECIS, FONT_QUALITY(quality), \
			   FIXED_PITCH | FF_DONTCARE, font->name)

	// We need a large font for the background title
	f(FONT_LARGE, font->charset, 600, FALSE, font_width, font_height*2);

    f(FONT_NORMAL, font->charset, fw_dontcare, FALSE, font_width, font_height);
    ...
    f(FONT_UNDERLINE, font->charset, fw_dontcare, TRUE, font_width, font_height);
	...
	f(FONT_BOLD, font->charset, fw_bold, FALSE, font_width, font_height);
    }
...
*/
void draw_caption(Context ctx, char* title)
{
	/*
	const size_t newsize = 100;
	size_t convertedChars = 0;
	size_t origsize = strlen(title) + 1;
	mbstowcs_s(&convertedChars, test, origsize, title, _TRUNCATE);
	*/
	COLORREF fg, bg;
	int nfg, nbg;
	int fnt_width;
	int char_width;
	//fg = RGB(.fg.r, tc.fg.g, tc.fg.b);
	nfg = ((term->default_attr & ATTR_FGMASK) >> ATTR_FGSHIFT);
	nbg = ((term->default_attr & ATTR_BGMASK) >> ATTR_BGSHIFT);
	bg = colours[nbg];
	...
	bg = RGB(r, g, b);
	SetTextAlign(ctx, TA_TOP | TA_RIGHT | TA_NOUPDATECP);
	SelectObject(ctx, fonts[FONT_LARGE]);
	SetTextColor(ctx, bg);

	SetBkMode(ctx, TRANSPARENT);
	fnt_width = char_width = font_width * 1;
	ExtTextOut(ctx, (term->cols*fnt_width), 0, NULL, NULL, title, strlen(title), NULL);
